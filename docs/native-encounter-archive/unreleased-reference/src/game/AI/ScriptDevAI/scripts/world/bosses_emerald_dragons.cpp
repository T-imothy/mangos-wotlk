...
struct boss_taerarAI : public boss_emerald_dragonAI
{
    ...
    void JustSummoned(Creature* summoned) override
    {
        if (m_uiShadesTimeoutTimer && summoned->GetEntry() == NPC_SHADE_OF_TAERAR)
            m_activeShades.insert(summoned->GetObjectGuid());
        boss_emerald_dragonAI::JustSummoned(summoned);
    }
    ...
    bool DoSpecialDragonAbility() override
    {
        ...
        {
            // Claim this wave before synchronous summon callbacks run.
            m_activeShades.clear();
    ...
    {
        // A timed-out wave can leave shades alive. Their later deaths, and
        // duplicate death callbacks, must not finish a newer banishment.
        if (m_uiShadesTimeoutTimer && pSummoned->GetEntry() == NPC_SHADE_OF_TAERAR &&
            m_activeShades.erase(pSummoned->GetObjectGuid()) && m_activeShades.empty())
            DoUnbanishBoss();
    }
