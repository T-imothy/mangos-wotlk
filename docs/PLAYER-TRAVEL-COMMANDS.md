...
Enter commands in chat. Full canonical commands also work in the addon PBTP protocol. Short aliases are case-insensitive. Existing points may be inside or outside; shared approaches require following the normal route. Eligibility, native entry requirements and cooldown still apply.
...
| Onyxia's Lair (level-60 version Classic/TBC; level-80 version WotLK) | `.tp onyxias_lair` | `.tp ONY` | Outside. Level-80 encounter in Wrath. |
...
| Utgarde Keep | `.tp utgarde_keep` | `.tp UK` | Outside. |
| The Nexus | `.tp nexus` | `.tp NEXUS` | Outside. |
| Azjol-Nerub | `.tp azjol_nerub` | `.tp AN` | Outside. |
| Ahn'kahet: The Old Kingdom | `.tp ahnkahet` | `.tp OK` | Inside. |
| Drak'Tharon Keep | `.tp draktharon_keep` | `.tp DTK` | Outside. |
| The Violet Hold | `.tp violet_hold` | `.tp VH` | Outside. |
| Gundrak | `.tp gundrak` | `.tp GD` | Outside. |
| Halls of Stone | `.tp halls_of_stone` | `.tp HOS` | Inside. |
| Halls of Lightning | `.tp halls_of_lightning` | `.tp HOL` | Inside. |
| The Oculus | `.tp oculus` | `.tp OCC` | Inside. |
| Utgarde Pinnacle | `.tp utgarde_pinnacle` | `.tp UP` | Outside. |
| The Culling of Stratholme | `.tp culling_stratholme` | `.tp COS` | Outside. |
| Trial of the Champion | `.tp trial_champion` | `.tp TOC5` | Outside. |
| The Forge of Souls | `.tp forge_of_souls` | `.tp FOS` | Inside. |
| Pit of Saron | `.tp pit_of_saron` | `.tp POS` | Inside. |
| Halls of Reflection | `.tp halls_of_reflection` | `.tp HOR` | Inside. |
| Naxxramas level 80 | `.tp naxxramas` | `.tp NAXX`, `.tp NAXX80` | Outside. |
| The Obsidian Sanctum | `.tp obsidian_sanctum` | `.tp OS` | Outside. |
| The Eye of Eternity | `.tp eye_of_eternity` | `.tp EOE` | Outside. Shared Nexus approach; follow the normal route to the Eye of Eternity. |
| Vault of Archavon | `.tp vault_archavon` | `.tp VOA` | Outside. Requires Wintergrasp ownership and no active battle. |
| Ulduar | `.tp ulduar` | `.tp ULDUAR` | Outside. |
| Trial of the Crusader | `.tp trial_crusader` | `.tp TOC` | Outside. |
| Icecrown Citadel | `.tp icecrown_citadel` | `.tp ICC` | Outside. |
| The Ruby Sanctum | `.tp ruby_sanctum` | `.tp RS` | Outside. |
