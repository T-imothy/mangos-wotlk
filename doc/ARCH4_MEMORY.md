...
* An optional pinned mimalloc v3.5.1 supplies the world executable's global C++ new/delete operators, including aligned/sized forms. `MANTECH_USE_MIMALLOC=OFF` builds with the original allocator. C malloc/free and allocations owned inside external DLLs are unchanged. Mimalloc is MIT licensed and its archive is pinned by commit and SHA-256 in `cmake/Arch4Allocator.cmake`.
