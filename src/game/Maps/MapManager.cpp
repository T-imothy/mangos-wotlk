...
void MapManager::Initialize()
{
    ...
    int const objectThreads = sWorld.getConfig(CONFIG_UINT32_MAP_OBJECT_THREADS);
    if (objectThreads > 0)
    {
...
void MapManager::CreateContinents()
{
    ...
    {
        std::vector<uint32> partitions{0};
        if (sWorld.getConfig(CONFIG_BOOL_CONTINENTS_INSTANCIATE))
        {
            if (id == 0)
                partitions = {MAP0_TOP_NORTH, MAP0_MIDDLE_NORTH, MAP0_IRONFORGE_AREA, MAP0_MIDDLE, MAP0_STORMWIND_AREA, MAP0_SOUTH};
            else if (id == 1)
                partitions = {MAP1_NORTH, MAP1_DUROTAR, MAP1_UPPER_MIDDLE, MAP1_LOWER_MIDDLE, MAP1_VALLEY, MAP1_ORGRIMMAR, MAP1_SOUTH};
        }
        ...
        for (uint32 instanceId : partitions)
        {
            ...
            // Map initialization touches shared pool, event, terrain, transport,
            // and persistent-state managers. Those systems are safe during
...
uint32 MapManager::GetContinentInstanceId(uint32 mapId, float x, float y, bool* transitionArea) const
{
    if (transitionArea)
...
void MapManager::ScheduleInstanceSwitch(Player* player, uint32 newInstanceId)
{
    std::lock_guard<std::mutex> guard(m_scheduledInstanceSwitchLock);
    m_scheduledInstanceSwitches[player] = newInstanceId;
}

void MapManager::SwitchPlayersInstances()
{
    std::map<Player*, uint32> switches;
    {
        std::lock_guard<std::mutex> guard(m_scheduledInstanceSwitchLock);
        switches.swap(m_scheduledInstanceSwitches);
    }

    for (auto const& entry : switches)
    {
        Player* player = entry.first;
        if (player && player->IsInWorld() && player->GetMap()->IsContinent() && player->GetInstanceId() != entry.second)
            player->SwitchInstance(entry.second);
    }
}
...
Map* MapManager::CreateMap(uint32 id, const WorldObject* obj)
{
    ...
    else
    {
        uint32 instanceId = 0;
        if (obj && id <= 1)
            instanceId = GetContinentInstanceId(id, obj->GetPositionX(), obj->GetPositionY());
        else if (obj && id == 609 && obj->IsPlayer()) // DeathKnightStart map - alliance and horde have own version
