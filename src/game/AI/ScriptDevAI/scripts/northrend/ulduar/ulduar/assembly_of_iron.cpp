...
struct boss_brundirAI : public BossAI
{
        ...
        m_isRegularMode(creature->GetMap()->IsRegularDifficulty())
    {
        ...
        AddTimerlessCombatAction(BRUNDIR_DEATH_CHECK, false);
    ...
    {
        SetActionReadyStatus(BRUNDIR_DEATH_CHECK, true);
    }
...
struct boss_molgeimAI : public BossAI
{
        ...
        m_isRegularMode(creature->GetMap()->IsRegularDifficulty())
    {
        ...
        AddTimerlessCombatAction(MOLGEIM_DEATH_CHECK, false);
    ...
    void JustPreventedDeath(Unit* /*attacker*/) override
    {
        SetActionReadyStatus(MOLGEIM_DEATH_CHECK, true);
    }
...
struct boss_steelbreakerAI : public BossAI
{
        ...
        m_isRegularMode(creature->GetMap()->IsRegularDifficulty())
    {
        ...
        AddTimerlessCombatAction(STEELBREAKER_DEATH_CHECK, false);
    }
    ...
    void JustPreventedDeath(Unit* /*attacker*/) override
    {
        SetActionReadyStatus(STEELBREAKER_DEATH_CHECK, true);
    }
    ...
    void ExecuteAction(uint32 action) override
    {
        switch (action)
        {
            ...
            case STEELBREAKER_PHASE_2_CHECK:
            {
                if (m_creature->GetAuraCount(SPELL_SUPERCHARGE) >= 1)
                {
            ...
            case STEELBREAKER_PHASE_3_CHECK:
            {
                if (m_creature->GetAuraCount(SPELL_SUPERCHARGE) >= 2)
                {
...
// 61887, 63486 - Lightning Tendrils
...
// 61888, 64637 - Overwhelming Power
...
struct RuneOfSummoningMolgeim : public AuraScript
{
    void OnPeriodicDummy(Aura* aura) const override
    {
        Unit* target = aura->GetTarget();
        if (!target->GetInstanceData() || target->GetInstanceData()->IsEncounterInProgress())
            target->CastSpell(target, 62020, TRIGGERED_OLD_TRIGGERED, nullptr, aura);
    }
};
...
void AddSC_boss_assembly_of_iron()
{
    ...
    RegisterSpellScript<RuneOfSummoningMolgeim>("spell_rune_of_summoning_molgeim");
}
