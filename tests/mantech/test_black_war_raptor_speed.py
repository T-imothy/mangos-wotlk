...
checkstart=castsource.index('    if (m_spellInfo->Id == 22721 && m_trueCaster->GetTypeId() == TYPEID_PLAYER)',castsource.index('SpellCastResult Spell::CheckCast('))
...
method+='\nint CheckRewardMount(Player* m_trueCaster, int id) { struct {int Id;} info{id}; auto m_spellInfo=&info;'+check+' return 0; }'
sql=(root/'sql/custom/world/20260911_01_black_war_raptor_reward.sql').read_text()
assert 'WHERE entry=18246' in sql and 'AllowableRace=-1' in sql and 'RequiredLevel=40' in sql and 'RequiredSkillRank=75' in sql and 'requiredhonorrank=0' in sql
player=(root/'src/game/Entities/Player.cpp').read_text()
assert 'if (IsInWorld() && level >= 40)' in player and 'SendItemQuerySingleResponse(18246)' in player
...
struct Player:Unit {int level=40,skill=75;int GetLevel(){return level;}int GetSkillValuePure(int id){assert(id==SKILL_RIDING);return skill;}};
struct Aura {
 Player player;int id=22721;
 struct {int m_auraname=32,m_amount=100;}m_modifier;
 struct Proto {int SpellIconID=0;}proto;
 Unit* GetTarget(){return &player;}int GetId(){return id;}Proto* GetSpellProto(){return &proto;}
